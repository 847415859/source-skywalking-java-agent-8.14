...
public class DispatcherHandlerHandleMethodInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    public void beforeMethod(EnhancedInstance objInst, Method method, Object[] allArguments, Class<?>[] argumentsTypes,
        ...
        LOGGER.info("DispatcherHandlerHandleMethodInterceptor.beforeMethod  Params:{}", allArguments);
    }
    ...
    public Object afterMethod(EnhancedInstance objInst, Method method, Object[] allArguments, Class<?>[] argumentsTypes,
        ...
        LOGGER.info("DispatcherHandlerHandleMethodInterceptor.afterMethod  Params:{}", allArguments);
        ServerWebExchange exchange = (ServerWebExchange) allArguments[0];
        ...
        LOGGER.info("Response Header: {}", exchange.getResponse().getHeaders());
        LOGGER.info("Find ME, TRACE-ID:{}", ContextManager.getGlobalTraceId());
        exchange.getResponse().getHeaders().add("DD-TRACE", ContextManager.getGlobalTraceId());
        LOGGER.info("Response Header: {}", exchange.getResponse().getHeaders());
        return monoReturn.doFinally(s -> {
            LOGGER.info("Find ME 2, TRACE-ID:{}", ContextManager.getGlobalTraceId());
            if (span != null) {
                maybeSetPattern(span, exchange);
                try {

                    ServerHttpResponse response = exchange.getResponse();
                    response.getHeaders().add("DD-TRACE-ID", ContextManager.getGlobalTraceId());
                    new StringTag(1001, "GATAWAY-TEST").set(span, "TK");
